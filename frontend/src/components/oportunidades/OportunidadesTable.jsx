      ...
      {numSelected > 0 ? (
        ...
        >
          {numSelected === 1
            ...
            : `${numSelected} contactos seleccionados`}
        </Typography>
      ) : (
      ...
      {numSelected > 0 ? (
        <Tooltip title="Eliminar Seleccionados">
          <button
            ...
            onClick={() => dispatch(toggleDeleteMany("contacto", selected))}
          >
...
export default function EnhancedTable() {
  ...
  const createData = (id, nombre, empresa, contacto, empleado, etapa) => {
    return {
      id,
      nombre,
      empresa,
  ...
  const createRows = (list) => {
    ...
    list.forEach((row) => {
      const contacto = `${row.contacto.nombre} ${row.contacto.apellido}`;
      const empleado = `${row.empleado.nombre} ${row.empleado.apellido}`;
  ...
  const rows = createRows(useSelector((store) => store.oportunidades.lista));
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell
                      ...
                      padding="none"
                    >
                      <Link to={`/oportunidades/${row.id}`}>{row.nombre}</Link>
                    </TableCell>
                    ...
                    <TableCell className="cellIcons">
                      ...
                      <button
                        onClick={() =>
                          dispatch(toggleDelete("contacto", row.id, row.nombre))
                        }
                      >
                        <FontAwesomeIcon
                          icon={faTrashAlt}
